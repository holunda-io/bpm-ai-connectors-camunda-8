...
def get_database_functions(llm: BaseLanguageModel, db: SQLDatabase) -> List[Callable]:
    def sql_query(query_template: str, **query_kwargs):
        """Takes a detailed and correct SQL query (potentially with template variables) and named parameters for every variable, returns a result from the database.
        ...
        return QuerySQLDataBaseTool(db=db).run(
    ...
    def sql_list_tables():
        ...
        return ListSQLDatabaseTool(db=db).run("").split(", ")

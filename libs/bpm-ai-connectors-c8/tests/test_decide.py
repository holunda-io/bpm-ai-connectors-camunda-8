...
def test_decide_string(zeebe_test_client: ZeebeTestClient):
    ...
    # when
    _, result = zeebe_test_client.create_process_instance_with_result("test_decide_string", variables=variables)

    # then
    assert result['decision'] == 'CANCEL_ORDER'


def test_decide_boolean(zeebe_test_client: ZeebeTestClient):
    # given
    variables = {
        "text": "I would like to cancel my order",
    ...
    # when
    _, result = zeebe_test_client.create_process_instance_with_result("test_decide_bool", variables=variables)
